...
export const useGalleryNavigationState = (): NavigationState => {
  ...
  if (!context) {
    throw new Error(
      'Attempted to use NavigationStateContext without a provider!'
    );
  }
...
const NavigationActionsContext = createContext<NavigationActions | undefined>(
  undefined
);
...
export const useGalleryNavigationActions = (): NavigationActions => {
  ...
  if (!context) {
    throw new Error(
      'Attempted to use NavigationActionsContext without a provider!'
    );
  }
...
const GalleryNavigationContextProvider = memo(
  ({ children, pathname, locationKey }: Props) => {
    ...
    const getVisitedPagesLength = useCallback(
      () => visitedPagesLength.current,
      []
    );
