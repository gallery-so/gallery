...
import { MultichainAuthenticateWallet } from './MultichainAuthenticateWallet';
...
export function MultichainWalletSelector({ connectionMode = AUTH, queryRef }: Props) {
  ...
  const setToPendingState = useCallback((walletName: WalletName) => {
    setIsPending(true);
  ...
  if (isPending && pendingWalletName) {
